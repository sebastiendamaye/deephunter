...
# Version 2.6
# This script will install DeepHunter v2.6 on a Ubuntu Server, with MariaDB as database, and Apache as web server with WSGI and a self-signed certificate.
set -euo pipefail
...
echo "                  |_|                                   "
echo ""
echo "            *** DeepHunter Installer v2.6 ***"
echo ""
echo ""
...
############
# Download DeepHunter v2.6
#
echo -n -e "[\033[90mINFO\033[0m] DOWNLOADING DEEPHUNTER .......................... " | tee -a /tmp/install.log
cd /tmp/
wget -q https://github.com/sebastiendamaye/deephunter/archive/refs/tags/v2.6.tar.gz >> /tmp/install.log 2>&1
tar xzf v2.6.tar.gz -C "${APP_PATH}" --strip-components=1 >> /tmp/install.log 2>&1
echo -e "[\033[32mdone\033[0m]" | tee -a /tmp/install.log
...
# Note: only the MITRE fixtures changed in v2.6 (MITRE ATT&CK v18). All other
# fixtures are shared with v2.5 and are loaded from the v2.5 fixtures folder.
...
./manage.py loaddata install/fixtures/v2.6/qm_mitretactic.json >> /tmp/install.log 2>&1
./manage.py loaddata install/fixtures/v2.6/qm_mitretechnique.json >> /tmp/install.log 2>&1
